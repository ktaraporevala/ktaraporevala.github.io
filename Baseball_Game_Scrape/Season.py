...
class Season:
    def __init__(self, folder_path: str, year: int):
        ...
        self.info.read_team_names(os.path.join(folder_path, f"TEAM{year}"))
    ...
    def run_season(self):
        for root, dirs, files in os.walk(self.folder_path):
            for file in files:
                if file[-4:].lower() in [f".ev{lg.lower()}" for lg in LeagueInfo.LEAGUES]:
...
def main():
    ...
    # season = Season(folder_2024, 2024)
    # test_filepath = r"C:\Users\ktara\Downloads\2024eve\2024HOU.EVA"
    # season.run_team_season(test_filepath, [33, 45, 77])
