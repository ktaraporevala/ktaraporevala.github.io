...
class Play:
    ...
    OUTS = ["K", "\d+"]  # TODO handle double plays
    ...
    def check_ignore_events(self):
        for event in Play.IGNORES:
            match = re.fullmatch(event, self.basic_play)
            if match is not None:
                assert not self.is_processed
                self.is_processed = True

    def check_independent_runner_events(self):

        for event in Play.ISOLATED_RUNNER_OUTS:
            match = re.fullmatch(event, self.basic_play)
            if match is not None:
                if "CS" in event:
                    runner_base = int(self.basic_play[self.basic_play.find("CS") + 2]) + \
                                  Play.ISOLATED_RUNNER_OUTS[event]
                elif "PO" in event:
                    runner_base = int(self.basic_play[self.basic_play.find("PO") + 2]) + \
                                  Play.ISOLATED_RUNNER_OUTS[event]
                else:
        ...
        for event in Play.STEALS:
            match = re.fullmatch(event, self.basic_play)
            if match is not None:
                end_base = self.basic_play[self.basic_play.find("SB")+2]
    ...
    def check_hitter_events(self):
        ...
        for event in Play.FORCE_OUTS:  # TODO make sure runner out is handled, give credit to runner out
            match = re.fullmatch(event, self.basic_play)
            if match is not None:
                runner_outs = re.search("\(\d\)", self.basic_play)
    ...
    def check_dependent_running_events(self):
        ...
        for running_event in self.runner_movements:
            ...
            for event in Play.BASE_ERRORS.keys():
                match = re.fullmatch(event, running_event)
                if match is not None:
    ...
    def run_play(self):
        self.check_ignore_events()
        self.check_hitter_events()
        self.check_independent_runner_events()
        if not self.is_processed:
            logging.warning(f"{self.basic_play} not recognized")
